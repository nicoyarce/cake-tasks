                ...
                <tr style="line-height: 50px; min-height: 50px; height: 50px;" id="{{ $propiedad->id }}">
                    ...
                    <td>
                        @if ($propiedad->id == 6)
                            {{ $propiedad->avance }}
                        @else
                            <input style="height:50px" disabled="" type="color" class="form-control"
                                value="{{ $propiedad->color }}">
                        @endif
                    </td>
                    <td>
                        <a href="{{ action('PropiedadesGraficoController@edit', $propiedad['id']) }}" type="button"
...
@section('modal-content')
    ...
    <form class="form-horizontal" method="POST"
        action="{{ action('PropiedadesGraficoController@update', $editar->id) }}">
        {{ csrf_field() }}
        {{ method_field('PUT') }}
            ...
            <div class="form-group">
                ...
                @if ($editar->id == 6)
                    <input type="text" class="form-control" id="avance"
                        @if ($editar->id != 6) readonly="" disabled="" @endif name="avance"
                        value="{{ $editar->avance }}">
                @else
                    <h4>{{ $editar->avance }}</h4>
                @endif
        ...
        @if ($editar->id != 6)
            <div class="form-group">
                ...
                <input style="height:50px" type="color" class="form-control" id="color" required name="color"
                    value="{{ $editar->color }}">
            </div>
        @endif
